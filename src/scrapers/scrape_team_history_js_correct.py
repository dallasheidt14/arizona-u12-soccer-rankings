"""
scrapers/scrape_team_history_js_correct.py
Purpose: Correct JavaScript-enabled scraper using /game-history endpoint
"""
...
def log(msg: str): print(f"[scrape_js_correct] {msg}")
...
def scrape_rankings_page_js_correct(division: str) -> List[Dict]:
...
def scrape_team_matches_js_correct(team_name: str, team_url: str, division: str) -> List[Dict]:
    """Scrape individual team matches using the /game-history endpoint"""
    if not team_url:
        return []
    
    # Extract team ID from URL (e.g., https://rankings.gotsport.com/teams/304588 -> 304588)
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Extract match data from game history page
            matches = page.evaluate("""
                () => {
                    ...
                    for (const table of tables) {
                        ...
                        // Look for table headers to identify match table
                        ...
                        if (firstRow) {
                            ...
                            const hasMatchHeaders = headers.some(h => 
                                h.includes('date') || h.includes('opponent') || h.includes('score') || 
                                h.includes('result') || h.includes('competition') || h.includes('team')
                            );
                            
                            if (hasMatchHeaders) {
                                console.log('Found match table with headers:', headers);
                                
                                for (let i = 1; i < rows.length; i++) { // Skip header
                                    const cells = rows[i].querySelectorAll('td');
                                    if (cells.length >= 3) {
                                        // Try different column arrangements
                                        let date = '', opponent = '', score = '', competition = '';
...
def run_division_js_correct(division: str) -> int:
    """Run correct JavaScript-enabled scraping for a division"""
    log(f"Starting correct JavaScript scraping for {division}")
    
    # Step 1: Scrape team list from rankings page
    teams_data = scrape_rankings_page_js_correct(division)
    ...
    # Step 2: Scrape match histories using /game-history endpoint
    ...
    for i, team_data in enumerate(teams_with_urls, 1):
        team_name = team_data["teamName"]
        team_url = team_data["teamUrl"]
        
        log(f"Scraping matches for {team_name} ({i}/{len(teams_with_urls)})")
        
        matches = scrape_team_matches_js_correct(team_name, team_url, division)
...
if __name__ == "__main__":
    ...
    code = run_division_js_correct(args.division)
