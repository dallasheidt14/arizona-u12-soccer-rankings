"""
scrapers/scrape_team_history_js_fixed.py
Purpose: Fixed JavaScript-enabled scraper with correct table parsing
"""
...
def log(msg: str): print(f"[scrape_js_fixed] {msg}")
...
def scrape_rankings_page_js_fixed(division: str) -> List[Dict]:
    """Scrape team rankings using JavaScript rendering with correct parsing"""
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Extract team data with better parsing
            teams = page.evaluate("""
                () => {
...
def scrape_team_matches_js_fixed(team_name: str, team_url: str, division: str) -> List[Dict]:
...
def run_division_js_fixed(division: str) -> int:
    """Run fixed JavaScript-enabled scraping for a division"""
    log(f"Starting fixed JavaScript scraping for {division}")
    
    # Step 1: Scrape team list from rankings page
    teams_data = scrape_rankings_page_js_fixed(division)
    ...
    # Save team list to bronze with correct column mapping
    ...
    # Step 2: Scrape match histories (only for teams with URLs)
    ...
    for i, team_data in enumerate(teams_with_urls, 1):
        ...
        matches = scrape_team_matches_js_fixed(team_name, team_url, division)
...
if __name__ == "__main__":
    ...
    code = run_division_js_fixed(args.division)
