"""
scrapers/scrape_team_history_js_api.py
Purpose: JavaScript scraper that intercepts API calls to find match data endpoints
"""
...
def log(msg: str): print(f"[scrape_js_api] {msg}")
...
def scrape_rankings_page_js_api(division: str) -> List[Dict]:
...
def scrape_team_matches_js_api(team_name: str, team_url: str, division: str) -> List[Dict]:
    """Scrape individual team matches by intercepting API calls"""
    if not team_url:
        return []
    ...
    with sync_playwright() as p:
        ...
        def handle_response(response):
            url = response.url
            if any(keyword in url.lower() for keyword in ['api', 'games', 'matches', 'history', 'results']):
                api_calls.append({
                    'url': url,
                    'status': response.status,
                    'method': response.request.method
                })
                log(f"API call detected: {response.request.method} {url} -> {response.status}")
        ...
        try:
                ...
                () => {
                    ...
                    for (const script of scripts) {
                        ...
                        if (content && (content.includes('match') || content.includes('game') || content.includes('result'))) {
                            console.log('Found script with potential match data:', content.substring(0, 200));
                        }
                    }
            ...
            if api_calls:
                log(f"Detected {len(api_calls)} API calls:")
                for call in api_calls:
                    log(f"  {call['method']} {call['url']} -> {call['status']}")
            else:
...
def run_division_js_api(division: str) -> int:
    """Run API-aware JavaScript scraping for a division"""
    log(f"Starting API-aware JavaScript scraping for {division}")
    
    # Step 1: Scrape team list from rankings page
    teams_data = scrape_rankings_page_js_api(division)
    ...
    for i, team_data in enumerate(test_teams, 1):
        ...
        matches = scrape_team_matches_js_api(team_name, team_url, division)
...
if __name__ == "__main__":
    ...
    code = run_division_js_api(args.division)
