...
# Note: Prediction module removed as part of cleanup
...
RANKINGS_FILE_PREFERENCE = os.getenv("RANKINGS_FILE_PREFERENCE", "v53e").split(",")
...
def load_rankings():
    """Load Rankings_v53_enhanced.csv for predictions."""
    rankings_path = DATA_DIR / "Rankings_v53_enhanced.csv"
    if rankings_path.exists():
        return pd.read_csv(rankings_path)
    else:
        # Fallback to other ranking files
        for fallback in ["Rankings_AZ_M_U12_2025_v53e.csv", "Rankings.csv"]:
...
def find_rankings_path(state: Optional[str], gender: Optional[str], year: Optional[str], division: Optional[str] = None, age_group: Optional[str] = None) -> Path:
    ...
    # Prefer v5.3E enhanced rankings only
    candidates = [
        DATA_DIR / f"Rankings_v53_enhanced{sfx}.csv",
        DATA_DIR / f"Rankings_v53_enhanced.csv",
        DATA_DIR / f"Rankings_AZ_M_U12_2025_v53e.csv",
...
# Prediction endpoint removed as part of cleanup
