# rankings/generate_team_rankings_v52b.py
"""
ARIZONA U12 RANKING LOGIC — V5.2b (Competitive Tuning)
...
6. Combine into PowerScore = 0.20*SAO + 0.20*SAD + 0.60*SOS
7. Output Rankings_v52b.csv sorted by PowerScore
"""
import pandas as pd
import numpy as np
import os
...
# ---- V5.2b Tuning Parameters (Competitive Tuning) ----
OFF_WEIGHT = 0.20
DEF_WEIGHT = 0.20
SOS_WEIGHT = 0.60

GOAL_DIFF_CAP = 6
PERFORMANCE_K = 0.10          # reduced from 0.20
...
def build_rankings_from_wide(wide_matches_csv: Path, out_csv: Path):
    ...
    # V5.2a: Compute strength-adjusted metrics (includes performance layer)
...
if __name__ == "__main__":
    # Example CLI usage:
    # python -m rankings.generate_team_rankings_v52b --in Matched_Games.csv --out Rankings_v52b.csv
    import argparse
    p = argparse.ArgumentParser()
    p.add_argument("--in", dest="in_path", required=True)
    p.add_argument("--out", dest="out_path", default="Rankings_v52b.csv")
    args = p.parse_args()
