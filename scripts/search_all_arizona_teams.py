#!/usr/bin/env python3
"""
Search through multiple pages of GotSport API to find all Arizona U11 Boys teams.
The API returns national rankings, so we need to search through pages to find Arizona teams.
"""
...
def search_all_arizona_teams():
    """Search through GotSport API pages to find all Arizona teams."""
    ...
    arizona_teams = []
    page = 1
    max_pages = 100  # Search first 100 pages
    
    print(f"Searching for Arizona U11 Boys teams through {max_pages} pages...")
        ...
        try:
            ...
            for team_record in teams_data:
                if isinstance(team_record, dict) and 'team_name' in team_record:
                    ...
                    # Check if this is an Arizona team
                    is_arizona = False
                    
                    # Check team name for Arizona indicators
                    if any(indicator in team_name.upper() for indicator in [
                        'AZ', 'ARIZONA', 'PHOENIX', 'SCOTTSDALE', 'TUCSON', 'MESA', 'CHANDLER', 
                        'GLENDALE', 'TEMPE', 'PEORIA', 'SURPRISE', 'GOODYEAR', 'AVONDALE', 
                        'BUCKEYE', 'CASA GRANDE', 'FLAGSTAFF', 'PRESCOTT', 'YUMA', 'KINGMAN', 
                        'LAKE HAVASU', 'RSL-AZ', 'FC ARIZONA', 'ARIZONA SOCCER', 'PHOENIX RISING', 
                        'PHOENIX PREMIER', 'SCOTTSDALE CITY', 'NORTH SCOTTSDALE', 'PIMA COUNTY', 
                        'CANYON DEL ORO', 'THUNDERBIRD', 'VAIL', 'MADISON', 'STATE 48', 'BALA', 
                        'LA ACADEMIA', 'AZ ARSENAL', 'NEXT LEVEL', 'ARIZONA SOCCER ACADEMY', 
                        'EXCEL SOCCER', 'PLAYMAKER', 'PARIS SAINT-GERMAIN', 'SC UNION', 'SPARTAK', 
                        'AYSO', 'SPARTANS', 'UNITED LATINOS'
                    ]):
                        is_arizona = True
                    
                    # Check club name for Arizona indicators
                    if not is_arizona and any(indicator in club_name.upper() for indicator in [
                        'AZ', 'ARIZONA', 'PHOENIX', 'SCOTTSDALE', 'TUCSON', 'MESA', 'CHANDLER', 
                        'GLENDALE', 'TEMPE', 'PEORIA', 'SURPRISE', 'GOODYEAR', 'AVONDALE', 
                        'BUCKEYE', 'CASA GRANDE', 'FLAGSTAFF', 'PRESCOTT', 'YUMA', 'KINGMAN', 
                        'LAKE HAVASU', 'RSL-AZ', 'FC ARIZONA', 'ARIZONA SOCCER', 'PHOENIX RISING', 
                    ...
                    if is_arizona:
                        ...
                        print(f"  Found AZ team: {team_name} (Club: {club_name})")
...
if __name__ == "__main__":
    try:
        ...
        teams = search_all_arizona_teams()
