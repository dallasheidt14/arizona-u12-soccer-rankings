# rankings/generate_team_rankings_v53.py
"""
ARIZONA U12 RANKING LOGIC — V5.3 (Form-Responsive Rankings with 30-Game Decay)
...
6. Compute SOS from comprehensive history (fallback to percentile)
7. Normalize metrics (min-max 0–1)
8. Combine into PowerScore = 0.20*SAO + 0.20*SAD + 0.60*SOS
9. Output Rankings_v53.csv sorted by PowerScore
"""
import pandas as pd
import numpy as np
import os
...
def compute_strength_adjusted_metrics(long_games: pd.DataFrame, base: pd.DataFrame) -> pd.DataFrame:
    """
    Compute strength-adjusted offense and defense metrics (V5.3).
...
def build_rankings_from_wide(wide_matches_csv: Path, out_csv: Path):
    ...
    # V5.3: Compute strength-adjusted metrics (includes Expected GD + Performance layer)
    print("Computing strength-adjusted offense/defense metrics with V5.3 form adjustments...")
    ...
    try:
        ...
        # Apply team name mapping to comprehensive history for master teams only
        # Keep original names for non-master opponents
        comp_hist["Team"] = comp_hist["Team"].map(lambda x: team_name_mapping.get(x, x))
    ...
    SOS_norm_raw = minmax_norm(adj["SOS_raw"])  # Now using actual opponent strength
    # Apply dynamic floor BEFORE stretching to preserve variation
    floor = np.nanpercentile(SOS_norm_raw, SOS_FLOOR_DYNAMIC_PERCENTILE)
    SOS_FLOOR_DYNAMIC = max(floor, SOS_FLOOR_MIN)
    SOS_norm_floored = SOS_norm_raw.clip(lower=SOS_FLOOR_DYNAMIC, upper=1.0)
    # Apply power stretch to emphasize top schedules
    SOS_norm_stretched = np.power(SOS_norm_floored, SOS_STRETCH_EXPONENT)
    # Re-normalize after stretch
    SOS_norm = minmax_norm(SOS_norm_stretched)
    print(f"Dynamic SOS floor: {SOS_FLOOR_DYNAMIC:.3f}")
    ...
    # Step 7: Compute Final PowerScore
    out["PowerScore"] = (
...
if __name__ == "__main__":
    # Example CLI usage:
    # python -m rankings.generate_team_rankings_v53 --in Matched_Games.csv --out Rankings_v53.csv
    import argparse
    p = argparse.ArgumentParser()
    p.add_argument("--in", dest="in_path", required=True)
    p.add_argument("--out", dest="out_path", default="Rankings_v53.csv")
    args = p.parse_args()
