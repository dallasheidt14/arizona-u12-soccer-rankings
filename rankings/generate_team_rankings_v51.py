...
6. Combine into PowerScore = 0.35*Off + 0.35*Def + 0.30*SOS
7. Output Rankings_v51.csv sorted by PowerScore
"""
import pandas as pd
import numpy as np
import os
...
def build_rankings_from_wide(wide_matches_csv: Path, out_csv: Path):
    ...
    out["PowerScore"] = (OFF_WEIGHT*out["Off_norm"] + DEF_WEIGHT*out["Def_norm"] + SOS_WEIGHT*out["SOS_norm"]).round(3)
...
if __name__ == "__main__":
    # Example CLI usage:
    # python -m rankings.generate_team_rankings_v51 --in Matched_Games.csv --out Rankings_v51.csv
    import argparse
    p = argparse.ArgumentParser()
    p.add_argument("--in", dest="in_path", required=True)
    p.add_argument("--out", dest="out_path", default="Rankings_v51.csv")
    args = p.parse_args()
