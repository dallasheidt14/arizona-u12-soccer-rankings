"""
Create U10 Master Team List from Bronze Data
...
def create_u10_master_from_bronze():
    """Create U10 master team list from bronze scraped data"""
    
    # Read bronze teams data
    bronze_path = "bronze/az_boys_u10_teams.csv"
    ...
    try:
        ...
        print("No games data found, creating master list without stats")
    ...
    for _, team in df_bronze.iterrows():
        ...
        team_canonical = team_name.lower()  # Use canonicalized name
        
        # Find games for this team
        if not df_games.empty:
            team_games = df_games[
                (df_games['Team A'] == team_canonical) | 
                (df_games['Team B'] == team_canonical)
            ]
                ...
                if game['Team A'] == team_canonical:
                    ...
                    if pd.notna(team_score) and pd.notna(opp_score):
                        if team_score > opp_score:
                            ...
                            ties += 1
                elif game['Team B'] == team_canonical:
                    # Team is Team B
        ...
        team_stats[team_name] = {
            'Team ID': f"u10_{team.get('Rank', len(df_bronze) - _)}",
            'Wins': wins,
            'Losses': losses,
            'Ties': ties,
    ...
    for idx, team in df_bronze.iterrows():
        team_name = team['Team Name']
        stats = team_stats.get(team_name, {})
        
        master_team = {
            'Team Name': team_name.lower(),  # Store canonicalized name
            'Club': team.get('Club', team_name.split()[0]),
            'Team ID': stats.get('Team ID', f"u10_{idx}"),
            ...
            'Team URL': team.get('TeamURL', ''),
            'Gender': 'M',
            'Age Group': 'U10'  # Match U12 structure exactly
        }
